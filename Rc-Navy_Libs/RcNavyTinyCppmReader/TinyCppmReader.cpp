...
#if (TINY_CPPM_READER_SINGLE_CHANNEL == 1)
static volatile uint16_t _ChWidthUs[1]; // Storage for a single channel is sufficient
#else
...
void TinyCppmReader::trackChId(uint8_t ChId)
{
  if((ChId >= 1) && (ChId <= TINY_CPPM_READER_CH_MAX))
  {
    _Cppm.ChIdx = ChId - 1;
  }
}
...
void TinyCppmReader::rcChannelCollectorIsr(void)
{
  ...
  if(TinyPinChange_RisingEdge(_VirtualPort, _CppmFrameInputPin))
  {
    ...
    else
    {
      ...
      {
#if (TINY_CPPM_READER_SINGLE_CHANNEL == 1)
        if(_ChIdx == _Cppm.ChIdx)
        {
          _ChWidthUs[0] = PulseDurationUs;
        }
#else
