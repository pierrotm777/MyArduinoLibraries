...
static void usbHandleCmd(String cmd)
{
  ...
  if (op == "d" ||
    ...
    uint8_t version = op.length() == 2 ? (uint8_t)(op.charAt(1) - '0')
                                       : (uint8_t)rest.toInt();
    if (version > 2) {
      Serial.print(F("Debug accepts d0, d1 or d2"));
      return;
    }
  ...
  if (op == "cn") {
    ...
    if (value != 8 && value != 16) {
      Serial.print(F("Contact number accepts only 8 or 16"));
      return;
    }
    if (inputExpanderReady) {
      Serial.print(F("Contact number is fixed by the connected expander"));
      return;
    }
