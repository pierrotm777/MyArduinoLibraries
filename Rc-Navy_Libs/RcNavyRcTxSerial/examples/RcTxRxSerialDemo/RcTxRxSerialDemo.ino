...
TinyCppmGen and RcTxserial are used to generate the PPM frame (PPM sum) on pin6 of an arduino UNO.
On the arduino UNO, the pin6 is connected to the pin2.
TinyCppmReader and RcRxSerial are the receiver part and extracts channels 1 to 4 and the message from channel 5.
...
TinyCppmGen et RcTxserial sont utilisés pour générer le train PPM (PPM sum) sur la broche6 d'un arduino UNO.
Sur l'arduino UNO, la broche6 est connectée à la broche2.
TinyCppmReader et RcRxSerial sont la partie réception et extraient les canaux 1 à 4 et le message texte depuis le canal 5.

                           Transmitter simulation (PPM Generation)      Extraction of Channels 1 to 4 and msg from Channel 5         
                            .----------------^-----------------.               .----------------^-----------------.
                                                                                         <TinyPinChange.h>
                                       <TinyCppmGen.h>                                    <TinyCppmReader.h>
                                         <RcTxPop.h>                                       <RcRxPop.h>
                                      .---------------.                                 .---------------.
                                      |               |      Pin6 connected to Pin2     |               |
  Values of Channels 1 to 4 --------->|  TinyCppmGen   |----> PPM Frame (5 channels) --->| TinyCppmReader |---------> Values of Channels 1 to 4
                                      |               |       (PPM sum signal)          |               |
                                      '---------------'                                 '---------------'
...
Mais, pour la démonstration, les 2 parties ont été groupées dans la meme sketch afin de n'utiliser qu'un seul arduino.
...
RcTxSerial MyRcTxSerial(&TinyCppmGen,    RC_TX_SERIAL_REPEAT3, 16, DATA_RC_CHANNEL); /* Create a Tx serial port with a tx fifo of 16 bytes on the channel#5 of the TinyCppmGen  (/!\ Data rate = 200 bauds /!\) */
static TinyCppmReader TinyCppmReader;
RcRxSerial MyRcRxSerial(&TinyCppmReader, RC_RX_SERIAL_NO_FILTER, DATA_RC_CHANNEL, 1); /* Create a Rx serial port on the channel#5 of the TinyCppmReader */
...
void loop()
{
  ...
  {
    TinyCppmGen.setChWidth_us(1, 500);
    TinyCppmGen.setChWidth_us(2, Width_us);       /* Channel#2 sweeps between 2000 and 1000 us */
    TinyCppmGen.setChWidth_us(3, 3000 - Width_us);/* Channel#3 sweeps between 1000 and 2000 us */
    TinyCppmGen.setChWidth_us(4, 2000);
    Width_us += Step_us;
  ...
  if(RxLen == MyRcRxSerial.msgAvailable(RxMsg, RX_MSG_LEN_MAX))
  {
    ...
    {
      Serial.print(F("Ch"));Serial.print(Ch);Serial.print(F("="));Serial.println(TinyCppmReader::width_us(Ch));
    }
