...
RcTxSerial MyRcTxSerial(&TinyCppmGen,    RC_TX_SERIAL_REPEAT0, 16, DATA_RC_CHANNEL); /* Create a Tx serial port with a tx fifo of 16 bytes on the channel#5 of the TinyPpmGen  (/!\ Data rate = 200 bauds /!\) */

static TinyCppmReader TinyCppmReader;
RcRxSerial MyRcRxSerial(&TinyCppmReader, RC_RX_SERIAL_NO_FILTER,   DATA_RC_CHANNEL); /* Create a Rx serial port on the channel#5 of the TinyPpmReader */
...
void setup()
{
  TinyCppmGen.begin(TINY_CPPM_GEN_POS_MOD, DATA_RC_CHANNEL, PPM_PERIOD_US); /* Generate 5 channels. Change TINY_PPM_GEN_POS_MOD to TINY_PPM_GEN_NEG_MOD for NEGative PPM modulation */
...
//  if(millis() - StartMs >= RC_TX_TIME_MS(PPM_PERIOD_US, RC_TX_SERIAL_REPEAT0, 6)) /* /!\ 250ms Clock used to not flood the serial link (Data rate # 200 Bauds) /!\ */
  {
