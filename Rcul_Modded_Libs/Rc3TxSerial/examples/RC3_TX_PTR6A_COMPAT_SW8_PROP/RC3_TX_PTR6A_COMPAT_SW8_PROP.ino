/*
  RC3 TX compatibility test - SW8 + PROP / 4 legacy nibbles
  --------------------------------------------------
  ...
  Payload:
    historical X-Any SW8+PROP packing:
      Message[0] = PROP
      Message[1] = SW8
    ...
    Total useful legacy payload = 4 nibbles.
*/
...
void setup()
{
  ...
  Serial.println(F("RC3 COMPAT SW8 + PROP TEST"));
  Serial.println(F("sendNibbleMsg(Message, 4, 1)"));
...
void loop()
{
  ...
  if(Rc3Tx.isReadyForTx())
  {
    ...
    /*
      ...
      Keep SW8 as the same incrementing counter used by the previous SW8
      test, and make PROP its complement so both fields are easy to verify.
    */
    const uint8_t Switch8 = Counter;
    const uint8_t Prop    = (uint8_t)~Counter;
    ...
    /*
      ...
      4 = useful legacy nibbles (PROP byte + SW8 byte).
      1 = rebuild historical RcTxSerial checksum on the bridge.
    */
    if(Rc3Tx.sendNibbleMsg(Message, 4, 1))
    {
      ...
      Serial.print(F(" SW8="));
      if(Switch8 < 0x10) Serial.print('0');
      Serial.println(Switch8, HEX);
