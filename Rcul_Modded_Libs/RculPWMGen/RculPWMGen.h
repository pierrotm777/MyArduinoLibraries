...
class RculPWMGen : public Rcul
{
  ...
  public:
    ...
    static int8_t         createInstance(void);   //Allocate dynamically an instance of a RculPWMGen object. Returns the object id.
    static uint8_t        createdInstanceNb(void);//Return the created instance(s) RculPWMGen object.
    static RculPWMGen *RculPWMGenById(uint8_t ObjIdx);
    static int8_t         getIdByPin(uint8_t Pin);
    static uint8_t        destroyInstance(uint8_t ObjIdx); //Deallocate dynamically an instance of a RculPWMGen object. Returns the object id.
