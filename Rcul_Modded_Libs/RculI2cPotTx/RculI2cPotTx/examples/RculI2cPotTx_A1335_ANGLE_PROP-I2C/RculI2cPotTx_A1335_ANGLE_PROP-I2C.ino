...
// ============================================================================
  ...
  /*
    P3022-V1-CW360 powered from 5 V.

    Use a 10 kOhm / 10 kOhm divider between sensor OUT and ANGLE_PIN:
      sensor OUT ---- 10k ----+---- ANGLE_PIN
                              |
                             10k
                              |
                             GND

    This converts approximately 0..5 V to 0..2.5 V.
    Adjust ANGLE_ADC_MIN and ANGLE_ADC_MAX after reading RAW_ANGLE
    at the two mechanical ends.
  */
...
static uint32_t RawAngleDebug = 0;
static uint32_t RawPropDebug = 0;
...
static uint32_t clampAdcValue(uint32_t Value,
                              uint32_t Minimum,
                              uint32_t Maximum)
{
...
static uint16_t scaleAngleAdcTo12Bits(uint32_t Raw)
{
  Raw = clampAdcValue(Raw, ANGLE_ADC_MIN, ANGLE_ADC_MAX);

  const uint32_t Span = ANGLE_ADC_MAX - ANGLE_ADC_MIN;
  if(Span == 0) return 0;

  return (uint16_t)(
      ((Raw - ANGLE_ADC_MIN) * 4095UL + (Span / 2UL)) / Span
  );
}

static uint8_t scalePropAdcTo8Bits(uint32_t Raw)
{
  Raw = clampAdcValue(Raw, PROP_ADC_MIN, PROP_ADC_MAX);

  const uint32_t Span = PROP_ADC_MAX - PROP_ADC_MIN;
  if(Span == 0) return 0;

  return (uint8_t)(
      ((Raw - PROP_ADC_MIN) * 255UL + (Span / 2UL)) / Span
  );
}
...
static void updateInputs(void)
{
  ...
  RawAngleDebug = (uint32_t)analogRead(ANGLE_PIN);
  Angle = scaleAngleAdcTo12Bits(RawAngleDebug);
  A1335Ok = true;

#endif

  RawPropDebug = (uint32_t)analogRead(PROP_PIN);
  Prop = scalePropAdcTo8Bits(RawPropDebug);
}
...
void setup()
{
  ...
  // Optional digital-potentiometer range. Defaults: MCP4561=4..252, DS3502=0..127.
...
void loop()
{
  ...
  if((uint32_t)(millis() - LastPrintMs) >= 1000UL)
  {
    ...
    Serial.print(RawAngleDebug);
    Serial.print(F(" ANGLE="));
    Serial.print(Angle);
    Serial.print(F(" ("));
    Serial.print((float)Angle * 360.0f / 4096.0f, 1);
    Serial.print(F(" deg) RAW_PROP="));
    Serial.print(RawPropDebug);
    Serial.print(F(" PROP="));
    Serial.print(Prop);
