...
RculI2cPotTxClass::RculI2cPotTxClass(RculI2cPotType_t PotType,
...
#endif
  _PotType(PotType),
  _PotChannel(((PotType == AD5282 || PotType == MCP4661) && PotChannel == 1) ? 1 : 0),
  _Address(0),
  ...
  _CallbackSynchroProfile(CALLBACK_SYNCHRO_NONE),
  _CallbackSyncPin(RCUL_I2C_POT_CALLBACK_SYNCHRO_NO_PIN),
  _CallbackPinState(LOW),
  _CallbackPulseStartUs(0),
  _CallbackLastValidPulseStartUs(0),
  _CallbackLastSyncUs(0),
  _CallbackLastPulseWidthUs(0),
  _CallbackCycleUs(0),
  _CallbackSyncOffsetUs(0),
  _CallbackSyncDueUs(0),
  _CallbackSyncPending(false),
...
RculI2cPotTxClass::RculI2cPotTxClass(
    RculI2cPotType_t PotType,
    RculI2cPotSyncCallbackPtr6A_t,
    uint8_t SyncPin,
    uint8_t PotChannel) :
  RculI2cPotTxClass(PotType, PotChannel)
{
  _SyncroMode = RCUL_I2C_POT_SYNCRO_MODE_CALLBACK;
  _CallbackSynchroProfile = CALLBACK_SYNCHRO_PTR_6A;
  _CallbackSyncPin = SyncPin;
  _CallbackSyncOffsetUs = RCUL_I2C_POT_CALLBACK_PTR_6A_SYNC_OFFSET_US;
}
...
uint8_t RculI2cPotTxClass::defaultAddress(void) const
{
  ...
  if(_PotType == MCP4661) return RCUL_I2C_POT_TX_MCP4661_ADDRESS;
...
uint16_t RculI2cPotTxClass::defaultMinWiper(void) const
{
  ...
  if(_PotType == MCP4661) return RCUL_I2C_POT_TX_MCP4661_WIPER_MIN;
...
uint16_t RculI2cPotTxClass::defaultMaxWiper(void) const
{
  ...
  if(_PotType == MCP4661) return RCUL_I2C_POT_TX_MCP4661_WIPER_MAX;
...
void RculI2cPotTxClass::loadDeviceDefaults(void)
{
  ...
  else if(_PotType == MCP4661) _DeviceMaxWiper = RCUL_I2C_POT_TX_MCP4661_MAX_WIPER;
...
bool RculI2cPotTxClass::writeMcp4661(uint16_t Wiper)
{
  Wiper = constrain(Wiper, (uint16_t)0,
                    (uint16_t)RCUL_I2C_POT_TX_MCP4661_MAX_WIPER);

  /*
   * MCP4661 volatile registers:
   *   0x00 = Wiper 0
   *   0x01 = Wiper 1
   *
   * Command byte:
   *   AD3:AD0 = register address
   *   C1:C0   = 00 (Write Data)
   *   D8      = bit 8 of the 9-bit wiper value
   *
   * This deliberately writes RAM only: the MCP4661 EEPROM is not
   * touched while RCUL symbols are transmitted.
   */
  const uint8_t Command =
      (uint8_t)(((_PotChannel == 1) ? 0x10 : 0x00) |
                ((Wiper >> 8) & 0x01));
...
bool RculI2cPotTxClass::writeDeviceWiper(uint16_t Wiper)
{
  ...
  if(_PotType == MCP4661) return writeMcp4661(Wiper);
  return writeMcp4561(Wiper);
}

/*
 * Initialisation d'un profil CALLBACK automatique.
 * Aucun acces GPIO n'est effectue depuis le constructeur global; pinMode()
 * n'est appele qu'apres begin(), lorsque l'environnement Arduino est pret.
 */
void RculI2cPotTxClass::beginCallbackSynchro(void)
{
  if((_SyncroMode != RCUL_I2C_POT_SYNCRO_MODE_CALLBACK) ||
     (_CallbackSynchroProfile == CALLBACK_SYNCHRO_NONE) ||
     (_CallbackSyncPin == RCUL_I2C_POT_CALLBACK_SYNCHRO_NO_PIN))
  {
    return;
  ...
  if(!RculI2cPotGetCallbackSynchroConfig(_CallbackSynchroProfile, Config))
  {
    return;
  }

  pinMode(_CallbackSyncPin, INPUT);
  _CallbackPinState = (uint8_t)digitalRead(_CallbackSyncPin);
  _CallbackPulseStartUs = micros();
  _CallbackLastValidPulseStartUs = 0;
  _CallbackLastSyncUs = 0;
  _CallbackLastPulseWidthUs = 0;
  _CallbackCycleUs = 0;
...
/*
 * Detection portable des profils CALLBACK automatiques.
 *
 * PTR-6A:
 *   GDO0 du CC2500 presente un motif 1 paquet court + 3 paquets longs.
 *   Le paquet court mesure environ 800..960 us. Le profil accepte
 *   600..1200 us et impose au moins 15 ms entre deux tops valides.
 *
 * Le top est genere au front qui termine l'impulsion active, donc ici au
 * front descendant de GDO0. Aucun Timer materiel ni ISR specifique n'est
 * utilise: digitalRead() + micros() suffisent sur Nano, ESP32 et Teensy.
 */
void RculI2cPotTxClass::processCallbackSynchro(void)
{
  if((_CallbackSynchroProfile == CALLBACK_SYNCHRO_NONE) ||
     (_CallbackSyncPin == RCUL_I2C_POT_CALLBACK_SYNCHRO_NO_PIN))
  {
    return;
  }
  ...
  /*
   * Retard de phase NON BLOQUANT:
   * le paquet court valide programme un top futur, puis process() continue
   * a surveiller GDO0 normalement. Le test signe reste valide au rebouclage
   * de micros() tant que le retard est tres inferieur a 2^31 us.
   */
  const uint32_t NowUs = micros();

  if(_CallbackSyncPending &&
     ((int32_t)(NowUs - _CallbackSyncDueUs) >= 0))
  {
    _CallbackSyncPending = false;
    _Synchro = 0xFF;
  }

  const uint8_t State = (uint8_t)digitalRead(_CallbackSyncPin);
  if(State == _CallbackPinState)
  {
    return;
  }

  const uint8_t PreviousState = _CallbackPinState;
  _CallbackPinState = State;

  if(State == Config.ActiveLevel)
  {
    _CallbackPulseStartUs = NowUs;
    return;
  }

  if(PreviousState != Config.ActiveLevel)
  {
    return;
  }

  const uint32_t WidthUs = (uint32_t)(NowUs - _CallbackPulseStartUs);

  if((WidthUs < Config.PulseMinUs) ||
     (WidthUs > Config.PulseMaxUs))
  {
    return;
  }

  if(_CallbackLastSyncUs != 0)
  {
    const uint32_t SinceLastMarkerUs =
        (uint32_t)(NowUs - _CallbackLastSyncUs);

    if(SinceLastMarkerUs < Config.MinSyncSpacingUs)
    {
      return;
    }
  }

  if(_CallbackLastValidPulseStartUs != 0)
  {
    _CallbackCycleUs =
        (uint32_t)(_CallbackPulseStartUs - _CallbackLastValidPulseStartUs);
  }

  _CallbackLastValidPulseStartUs = _CallbackPulseStartUs;
  _CallbackLastPulseWidthUs =
      (uint16_t)((WidthUs > 65535UL) ? 65535UL : WidthUs);

  /*
   * Repere RF absolu = front descendant du paquet court valide.
   * Offset=0 : top immediat, comportement V1.10.4.
   * Offset>0 : top programme sans delay() ni attente active.
   */
  _CallbackLastSyncUs = NowUs;

  if(_CallbackSyncOffsetUs == 0)
  {
    _CallbackSyncPending = false;
    _Synchro = 0xFF;
  }
  ...
  {
    _CallbackSyncDueUs = NowUs + (uint32_t)_CallbackSyncOffsetUs;
    _CallbackSyncPending = true;
  }
}
 ...
 *
 * BY_CALLBACK:
 *   - profil NONE: le sketch appelle syncPulse();
 *   - profil PTR-6A: process() extrait automatiquement le top de GDO0.
 */
...
RculI2cPotTxClass::getCallbackSynchroProfile(void) const
{
  return _CallbackSynchroProfile;
}

uint8_t RculI2cPotTxClass::getCallbackSyncPin(void) const
{
  return _CallbackSyncPin;
}

uint16_t RculI2cPotTxClass::getCallbackLastPulseWidthUs(void) const
{
  return _CallbackLastPulseWidthUs;
}

uint32_t RculI2cPotTxClass::getCallbackCycleUs(void) const
{
  return _CallbackCycleUs;
}

bool RculI2cPotTxClass::setCallbackSyncOffsetUs(uint16_t OffsetUs)
{
  if(_CallbackSynchroProfile == CALLBACK_SYNCHRO_NONE)
  {
    return false;
  ...
  _CallbackSyncOffsetUs = OffsetUs;
  _CallbackSyncPending = false;
  return true;
}

uint16_t RculI2cPotTxClass::getCallbackSyncOffsetUs(void) const
{
  return _CallbackSyncOffsetUs;
}
    ...
    const uint16_t *Table) const
{
  ...
  if((_PotType == AD5282) || (_PotType == MCP4661))
  {
...
void RculI2cPotTxClass::printInfo(Stream &Out)
{
  ...
  else if(_PotType == MCP4661) Out.println(F("MCP4661"));
  ...
  switch(_SyncroMode)
  {
    ...
    case RCUL_I2C_POT_SYNCRO_MODE_CALLBACK:
      if(_CallbackSynchroProfile == CALLBACK_SYNCHRO_PTR_6A)
        Out.println(F("CALLBACK / PTR-6A"));
      else
  ...
  if((_SyncroMode == RCUL_I2C_POT_SYNCRO_MODE_CALLBACK) &&
     (_CallbackSynchroProfile != CALLBACK_SYNCHRO_NONE))
  {
    Out.print(F("Sync GPIO    : ")); Out.println(_CallbackSyncPin);
    Out.print(F("Sync offset  : ")); Out.print(_CallbackSyncOffsetUs); Out.println(F(" us"));
  }
