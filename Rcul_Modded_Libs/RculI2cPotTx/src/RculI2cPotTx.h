...
#define RCUL_I2C_POT_TX_PATCH                   7
#define RCUL_I2C_POT_TX_VERSION_STRING          "1.10.7"
#define RCUL_I2C_POT_TX_VERSION_NUM             0x010A07
...
struct RculI2cPotSyncCallbackPtr6A_t {};
...
#define RCUL_I2C_POT_SYNCRO_BY_CALLBACK_PTR_6A   RculI2cPotSyncCallbackPtr6A_t()
...
#define RCUL_I2C_POT_TX_MCP4661_ADDRESS         0x28
...
#define RCUL_I2C_POT_TX_MCP4661_MAX_WIPER       256
...
#define RCUL_I2C_POT_TX_MCP4661_WIPER_MIN       4
#define RCUL_I2C_POT_TX_MCP4661_WIPER_MAX       252
#define RCUL_I2C_POT_TX_AD5282_CHANNEL_0        0
#define RCUL_I2C_POT_TX_AD5282_CHANNEL_1        1
#define RCUL_I2C_POT_TX_MCP4661_CHANNEL_0       0
#define RCUL_I2C_POT_TX_MCP4661_CHANNEL_1       1
...
class RculI2cPotTxClass : public Rcul
{
  private:
    ...
    RculI2cPotCallbackSynchroProfile_t _CallbackSynchroProfile;
    uint8_t _CallbackSyncPin;
    uint8_t _CallbackPinState;
    uint32_t _CallbackPulseStartUs;
    uint32_t _CallbackLastValidPulseStartUs;
    uint32_t _CallbackLastSyncUs;
    uint16_t _CallbackLastPulseWidthUs;
    uint32_t _CallbackCycleUs;
    uint16_t _CallbackSyncOffsetUs;
    uint32_t _CallbackSyncDueUs;
    bool _CallbackSyncPending;
    ...
    bool writeMcp4661(uint16_t Wiper);
    bool writeDeviceWiper(uint16_t Wiper);
    uint16_t widthToWiper(uint16_t Width_us) const;

    void beginCallbackSynchro(void);
    void processCallbackSynchro(void);
  ...
  public:
    ...
    /*
     * CALLBACK profile PTR-6A:
     * le 3e argument est la broche GDO0. Le top est extrait automatiquement
     * par process(), sans Timer materiel ni ISR specifique a la plateforme.
     */
    RculI2cPotTxClass(RculI2cPotType_t PotType,
    ...
    RculI2cPotCallbackSynchroProfile_t getCallbackSynchroProfile(void) const;
    uint8_t getCallbackSyncPin(void) const;
    uint16_t getCallbackLastPulseWidthUs(void) const;
    uint32_t getCallbackCycleUs(void) const;

    /*
     * Retard non bloquant du top CALLBACK automatique apres la fin
     * de l'impulsion valide. 0 us conserve le comportement historique.
     */
    bool setCallbackSyncOffsetUs(uint16_t OffsetUs);
    uint16_t getCallbackSyncOffsetUs(void) const;
 ...
 * pour creer soi-meme une instance DS3502, MCP4561, AD5282 ou MCP4661 du meme nom.
 */
