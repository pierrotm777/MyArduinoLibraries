...
void RculCrsfSerial::begin(uint32_t baud)
{
    if (baud != 0)
        _port.begin(baud);
    else
...
void RculCrsfSerial::loop()
{
    handleSerialIn();
}

void RculCrsfSerial::handleSerialIn()
{
...
void RculCrsfSerial::handleByteReceived()
{
    bool reprocess;
    do
    {
...
void RculCrsfSerial::checkPacketTimeout()
{
...
void RculCrsfSerial::checkLinkDown()
{
...
void RculCrsfSerial::processPacketIn(uint8_t len)
{
...
void RculCrsfSerial::shiftRxBuffer(uint8_t cnt)
{
...
void RculCrsfSerial::packetChannelsPacked(const crsf_header_t *p)
{
...
void RculCrsfSerial::packetLinkStatistics(const crsf_header_t *p)
{
...
void RculCrsfSerial::packetGps(const crsf_header_t *p)
{
...
void RculCrsfSerial::write(uint8_t b)
{
    _port.write(b);
}

void RculCrsfSerial::write(const uint8_t *buf, size_t len)
{
    _port.write(buf, len);
}

void RculCrsfSerial::queuePacket(uint8_t type, const void *payload, uint8_t len)
{
    if (getPassthroughMode())
        return;
 ...
 */
void RculCrsfSerial::queuePacketChannels()
{
...
*/
void RculCrsfSerial::setPassthroughMode(bool val, uint32_t passthroughBaud)
{
    if (val)
    {
...
uint16_t RculCrsfSerial::width_us(uint8_t Ch)
{
...
uint8_t RculCrsfSerial::RculIsSynchro(uint8_t ClientIdx /*= RCUL_DEFAULT_CLIENT_IDX*/)
{
  return(isSynchro(ClientIdx));
}

uint16_t RculCrsfSerial::RculGetWidth_us(uint8_t Ch)
{
  return(width_us(Ch));
}

void RculCrsfSerial::RculSetWidth_us(uint16_t Width_us, uint8_t Ch /*= RCUL_NO_CH*/)
{
