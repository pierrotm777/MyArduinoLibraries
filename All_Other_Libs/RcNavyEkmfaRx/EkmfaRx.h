...
#include <string.h>
#elif not defined(ARDUINO_ARCH_RP2040)
...
#if defined(ARDUINO_ARCH_ESP32)
enum EkmfaStorageMode : uint8_t
{
    EKMFA_STORAGE_EEPROM      = 0, /* ESP32 default: uses EEPROM emulation. Sketch must call EEPROM.begin(size). */
    EKMFA_STORAGE_PREFERENCES = 1  /* Optional ESP32 backend: uses NVS Preferences namespace. */
};
#endif
...
class EkmfaRxClass
{
  private:
    EkmfaRxSt_t          Ekmfa;
#if defined(ARDUINO_ARCH_ESP32)
    EkmfaStorageMode     StorageMode;
    char                 PrefNamespace[16]; /* ESP32 Preferences namespace: 15 chars max + null */
#endif
    uint16_t             EepBaseAddr;
    void               (*UpdateNvCallback)(void); /* Optional callback called after EEPROM duration write. */
  public:
    EkmfaRxClass();
    ...
    void                 setPreferencesNamespace(const char *Namespace); /* Kept for compatibility. ESP32: selects Preferences backend. AVR/Teensy: no-op. */
    void                 setStorageEEPROM(uint16_t EepBaseAddr);          /* ESP32: select EEPROM emulation backend. Sketch must call EEPROM.begin(size). AVR/Teensy: sets base address. */
    void                 setStoragePreferences(const char *Namespace);     /* ESP32: select Preferences/NVS backend. AVR/Teensy: no-op. */
    void                 setUpdateNvCallback(void (*Callback)(void)); /* Optional: same Nv repair callback as RcButtonRx. */
#if (EKMFA_RX_DEBUG > 0)
    void                 debugProtocol(uint8_t OffOn);
#endif
#ifndef EKMFA_DEFAULT_DURATIONS
    void                 setEepBaseAddr(uint16_t EepBaseAddr);
    uint16_t             getEepTotalSize(void);
    uint16_t             getEepDurationMs(uint8_t WordIdx);
