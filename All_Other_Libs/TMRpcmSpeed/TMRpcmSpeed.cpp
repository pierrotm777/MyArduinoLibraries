...
boolean TMRpcmSpeed::wavInfo(const char* filename){
...
void TMRpcmSpeed::quality(boolean q){
	if(!playing){	bitWrite(optionByte,6,q); } //qual = q; }
}

void TMRpcmSpeed::stopPlayback(){
	playing = 0;
...
void TMRpcmSpeed::loop(boolean set){
...
Prevents a whole lot more #if defined statements */
	...
	boolean TMRpcmSpeed::seek( unsigned long pos ){
		return sFile.seek(pos);
	}

	unsigned long TMRpcmSpeed::fPosition( ){
		return sFile.position();
	}

	boolean TMRpcmSpeed::ifOpen(){
		if(sFile){ return 1;}
	}
...
void TMRpcmSpeed::play(const char* filename){
	play(filename,0);
}


void TMRpcmSpeed::play(const char* filename, unsigned long seekPoint){
...
void TMRpcmSpeed::volume(char upDown){
...
void TMRpcmSpeed::setVolume(char vol) {
    volMod = vol - 4 ;
}

void TMRpcmSpeed::speed(int sampleRate){ 					// Speed MOD
...
void TMRpcmSpeed::loop(boolean set, boolean which){
	if(!which){ bitWrite(optionByte,3,set); }
	else{	    bitWrite(optionByte,2,set); }
}

void TMRpcmSpeed::play(const char* filename, boolean which){
...
void TMRpcmSpeed::play(const char* filename, unsigned long seekPoint, boolean which){
...
void TMRpcmSpeed::ramp(boolean wBuff){
...
void TMRpcmSpeed::volume(char upDown,boolean which){
...
void TMRpcmSpeed::setVolume(char vol, boolean which) {
    volMod = vol - 4 ;
}
...
void TMRpcmSpeed::stopPlayback(boolean which){
	if(!playing && !playing2){
		*TIMSK[tt] &= ~( togByte | _BV(TOIE1) );
	}
	if(!which){
...
boolean TMRpcmSpeed::isPlaying(boolean which){
...
byte TMRpcmSpeed::getInfo(const char* filename, unsigned char* tagData, byte infoNum){
	byte gotInfo = 0;
...
byte TMRpcmSpeed::listInfo(const char* filename, unsigned char* tagData, byte infoNum){
...
byte TMRpcmSpeed::id3Info(const char* filename, unsigned char* tagData, byte infoNum){
	return metaInfo(1, filename, tagData, infoNum);
}


#if !defined (SDFAT)
boolean TMRpcmSpeed::searchMainTags(File xFile, const char *datStr){
	xFile.seek(36);
#else
unsigned long TMRpcmSpeed::searchMainTags(SdFile xFile, const char *datStr){
	xFile.seekSet(36);
#endif
		boolean found = 0;
...
byte TMRpcmSpeed::metaInfo(boolean infoType, const char* filename, unsigned char* tagData, byte whichInfo){
...
void TMRpcmSpeed::finalizeWavTemplate(const char* filename){
	disable();
...
void TMRpcmSpeed::createWavTemplate(const char* filename, unsigned int sampleRate){
	disable();
...
void TMRpcmSpeed::startRecording(char *fileName, unsigned int SAMPLE_RATE, byte pin){
	startRecording(fileName,SAMPLE_RATE,pin,0);
}

void TMRpcmSpeed::startRecording(char *fileName, unsigned int SAMPLE_RATE, byte pin, byte passThrough){
...
void TMRpcmSpeed::stopRecording(char *fileName){
