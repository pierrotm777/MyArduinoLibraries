...
//PPMReader CppmReader = PPMReader();à tester

/* Public functions */
PPMReader::PPMReader(void) /* Constructor */
{
}

uint8_t PPMReader::attach(uint8_t CppmInputPin)
{
  uint8_t Ret = 0;
...
// uint8_t PPMReader::detach(void)
// {
  // suspend();
  // return(TinyPinChange_UnregisterIsr(_CppmFrameInputPin, PPMReader::rcChannelCollectorIsr));
// }

void PPMReader::trackChId(uint8_t ChId)
{
...
uint8_t PPMReader::detectedChannelNb(void)
{ 
  return(_ChIdxMax); /* No need to mask/unmask interrupt (8 bits) */
}

uint16_t PPMReader::width_us(uint8_t Ch)
{
  uint16_t Width_us = 1500;

  if((Ch >= 1) && (Ch <= PPMReader::detectedChannelNb()))
  {
    Ch--;
...
uint16_t PPMReader::cppmPeriod_us(void)
{
...
uint8_t PPMReader::isSynchro(uint8_t ClientIdx /*= 7*/)
{
  uint8_t Ret;
...
uint8_t PPMReader::RculIsSynchro(uint8_t ClientIdx /*= RCUL_DEFAULT_CLIENT_IDX*/)
{
  return(isSynchro(ClientIdx));
}

uint16_t PPMReader::RculGetWidth_us(uint8_t Ch)
{
  return(width_us(Ch));
}

void PPMReader::RculSetWidth_us(uint16_t Width_us, uint8_t Ch /*= RCUL_NO_CH*/)
{
...
// void PPMReader::suspend(void)
// {
  // TinyPinChange_DisablePin(_CppmFrameInputPin);
  // _ChIdx = (CPPM_READER_CH_MAX + 1);
// }

// void PPMReader::resume(void)
// {
...
/* ISR */
void PPMReader::rcChannelCollectorIsr(void)
{
