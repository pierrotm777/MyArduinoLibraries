...
English:
=======
This sketch demonstrates how to use the <RcBusRx> library when it is configured for SRXL2.
...
Just connect the Receiver SRXL2 signal to the Rx pin of the Serial1 UART on the arduino.

Francais:
========
Ce sketch demontre comment utiliser la bibliotheque <RcBusRx> quand elle est configuree en SRXL2.
...
Connecter simplement le signal SRXL2 du recepteur a la broche Rx de l'UART Serial1 de l'arduino.
...
void setup()
{
  ...
  Serial.println(F("RcBusRx configured in SRXL2 receiver\n"));
  
  Serial1.begin(SRXL2_RX_SERIAL_CFG); /* Choose your serial and select xxxx__RX_SERIAL_CFG (list in RcBusRx.h) where xxxx is the protocol */
  
  RcBusRx.serialAttach(&Serial1);    /* Then, attach the SRXL2 receiver to this Serial1 */
  RcBusRx.setProto(RC_BUS_RX_SRXL2); /* Select here the serial protocol */
}
