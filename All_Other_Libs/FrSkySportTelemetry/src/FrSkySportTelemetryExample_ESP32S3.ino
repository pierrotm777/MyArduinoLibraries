/*
  ...
  Note that you need ESP32 S3? Teensy LC/3.x/4.x, ESP8266, ATmega2560 (Mega) or ATmega328P based (e.g. Pro Mini, Nano, Uno) board and FrSkySportTelemetry library for this example to work
*/
...
#define SPORT_PIN 1  // GPIO S.Port single-wire ESP32-S3


#define INPUT_PIN_C 43  //      //UART_RX2 - Sound Next(PWM) / I2C SCL Radiolink
#define INPUT_PIN_D 1   //      //UART_TX2
...
void setup()
{
  Serial.begin(115200);
  delay(1000);
  Serial.println("Test Télémétrie S-Port - Tx=43 /Rx=1");
  // Configure the telemetry serial port and sensors (remember to use & to specify a pointer to sensor)
  telemetry.begin(FrSkySportSingleWireSerial::SERIAL_2_S3, SPORT_PIN, &ass, &esc, &fcs, &flvss1, &flvss2, &gas, &gps, &rpm, &sp2uart, &vario);
}
