/*
This sketch demonstrates how to use <RculPWMReader> library to get RC pulses from a receiver and to use <RculPWMGen> library to drive 2 servos.
...
RculPWMReader  RxChannelPulse;       /* RxChannelPulse is an objet  of RculPWMReader type */
RculPWMGen ServoMotor[SERVO_NB]; /* Table Creation for 2 objets of RculPWMGen type */
...
void setup()
{
  ...
  Serial.begin(9600);
  Serial.print(F("RculPWMReader library V"));Serial.print(SOFT_RC_PULSE_IN_VERSION);Serial.print(F("."));Serial.print(SOFT_RC_PULSE_IN_REVISION);Serial.print(" demo"); /* For arduino UNO which has an hardware UART, display the library version in the console */
#endif
...
static uint16_t Width_us=NEUTRAL_US; /* Static to keep the value at the next loop */
  ...
  if(RxChannelPulse.available())
  {
    ...
    RculPWMGen::refresh(NOW); /* NOW argument (=1) allows to synchronize outgoing pulses with incoming pulses */
  ...
  else
  {
    ...
    if(millis()-RxPulseStartMs>=PULSE_MAX_PERIOD_MS)
    {
      ...
      RculPWMGen::refresh(NOW); /* Immediate refresh of outgoing pulses */
      RxPulseStartMs=millis(); /* Restart the Chrono for Pulse */
    }
  }
